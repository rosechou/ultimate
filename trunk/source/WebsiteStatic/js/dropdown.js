...
function initSpinners() {
    ...
    */
    ...
    } else if (_SPINNER.task.selected) _SPINNER.tool.selected.onselect();
    else {
        appendSettings();
...
function onToolSelect(self) {
    ...
    setTimeout(function (c) {
        Arr.foreach(c, function (k, v) {
            if (editorHasCode() && _ITEMS[v].language == _CUR_LANG) _ITEMS[v].onselect();
        });
    }, 50, self.selected.children);
    ...
    if (tool && task && task.settings && task.settings.length > 0) {
        ...
        Arr.foreach(task.settings, function (k, v) {
            ...
            if (setting.type == 'DROPDOWN') {
                ...
                Arr.foreach(setting.items, function (k, v) {
                    _ITEMS[v].insert();
                });
                $('#' + setting.value.replace(/\s+/g, '')).addClass('selected');
                _SPINNER[setting.id].selected = _ITEMS[setting.value];
                box.lastChild.onmouseover = function () {
                    this.firstChild.firstChild.innerHTML = _SPINNER[this.firstChild.id].selected.name;
                };
                box.lastChild.onclick = box.lastChild.onmouseover;
                box.lastChild.onmouseout = function () {
                    this.firstChild.firstChild.innerHTML = this.dataset.label;
                };
            }
...
function getInputTemplate(id, label, value) {
    ...
    */
    ...
    $(div.children[1]).focus(function () {
        this.select();
    });
    div.children[1].onchange = function () {
...
function getSliderTemplate(id, label, value, onchange, min, max, unit) {
    ...
    */
    ...
    div.children[1].onchange = function (callback, unit) {
        return function () {
    ...
    div.onmousewheel = function (e) {
        ...
        var d = e.detail ? e.detail * (-1) : e.wheelDelta;
        el.value = parseInt(el.value) + ((d > 0) ? 1 : -1);
        el.onchange();
    };
...
function getElementFromSetting(setting) {
    ...
    switch (setting.type) {
        case 'check':
        case 'BOOLEAN':
            div = getCheckTemplate(setting.id, setting.label, function () {
...
Item.prototype = {
    ...
    onselect: function (self) {
        ...
        if (self.children && self.children.length > 0) {
            ...
            Arr.foreach(self.children, function (k, v) {
                _ITEMS[v].insert(single);
            });
            document.getElementById(childSpinner.id).style.display = 'block';
            if (single && childSpinner.id == 'task') {
                firstChild.onselect(firstChild);
        ...
        setTimeout(function () {
            $(spinner).removeClass('disappear active');
        }, 300);
    }
};
