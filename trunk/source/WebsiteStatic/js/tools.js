...
function clearResults() {
    ...
    if (_ANIMATE && !$('#content').hasClass('no-messages')) {
        ...
        setTimeout(function () {
            $('#content')[0].className = 'no-messages';
        }, 500);
...
function removeAnnotation(i) {
    var annot = [];
    Arr.foreach(_EDITOR.session.getAnnotations(), function (k, v) {
        if (k != i) annot.push(v);
    });
...
function addResults(arr, now) {
    if (!now) {
        setTimeout(addResults, _ANIMATE * 500, arr, true);
        return;
    }
    ...
    Arr.foreach(arr, function (k, v) {
        annot.push(addResult(v));
    });

    _EDITOR.getSession().setAnnotations(annot);

    $('.messages-item .close').each(function () {
        this.onclick = removeResult;
    });
...
function getResultTemplate(res) {
    ...
    if (res.startLNr != -1 && res.endLNr != -1) {
        ...
        } else if (res.startCol == res.endCol) {
            scopeCol = res.startCol;
        } else if (res.startCol == -1) {
            res.startCol = 0;
            scopeCol = '0 - ' + res.endCol;
        } else if (res.endCol == -1) {
            res.endCol = 0;
            res.endLNr++;
            scopeCol = res.startCol;
    ...
    msg.onclick = function (r) {
        return function () {
            highlightCode(r, r.logLvl);
            _EDITOR.focus();
        };
    }(res);
...
function highlightCode(r, c) {
    ...
    if (r.startCol + r.endCol == -2) {
        endLn++;
    } else
    // one line selection, when columns are identical
    if (startLn == endLn && startCol == endCol) {
        startCol = 0;
        endCol = 0;
        endLn++;
    }
